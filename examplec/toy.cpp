...
#define ANIMATION_ID_PB_HELP 44.0
...
#define MSG_NO_NEXT_POSITION {20.0f * TILE_PITCH - 2.0f * PUSH_BUTTON_RADIUS - MSG_NO_NEXT_WIDTH / 2.0f, 6.5f * TILE_PITCH}

#define MSG_HELP_WIDTH (TILE_PITCH * 20.0f)
#define MSG_HELP_HEIGHT (MSG_HELP_WIDTH / 2.4f)
#define MSG_HELP_POSITION {5.0f * TILE_PITCH, 3.4f * TILE_PITCH}

#define MSG_SOLVED_WIDTH (TILE_PITCH * 11.667f)
#define MSG_SOLVED_HEIGHT (MSG_SOLVED_WIDTH / 11.2f)
#define MSG_SOLVED_POSITION {20.0f * TILE_PITCH - 2.0f * PUSH_BUTTON_RADIUS - MSG_SOLVED_WIDTH / 2.0f, 6.5f * TILE_PITCH}

#define MSG_STATUS_WIDTH (TILE_PITCH * 5.0f)
#define MSG_STATUS_HEIGHT (MSG_STATUS_WIDTH / 4.8f)
#define MSG_STATUS_POSITION {5.0f * TILE_PITCH, -1.0f * TILE_PITCH}

#define COUNT_WIDTH (TILE_PITCH * 0.7333333f)
#define COUNT_HEIGHT (MSG_STATUS_HEIGHT)
#define COUNT_POSITION {5.0f * TILE_PITCH, -1.0f * TILE_PITCH}
...
Toy::Toy()
    ...
    , m_msg_no_next(new MessagePanel(MSG_NO_NEXT_WIDTH, MSG_NO_NEXT_HEIGHT, MSG_NO_NEXT_POSITION))
    , m_msg_no_more(new MessagePanel(MSG_NO_NEXT_WIDTH, MSG_NO_NEXT_HEIGHT, MSG_NO_NEXT_POSITION))
    , m_msg_help(new MessagePanel(MSG_HELP_WIDTH, MSG_HELP_HEIGHT, MSG_HELP_POSITION))
    , m_msg_about(new MessagePanel(MSG_HELP_WIDTH, MSG_HELP_HEIGHT, MSG_HELP_POSITION))
    , m_msg_solved(new MessagePanel(MSG_SOLVED_WIDTH, MSG_SOLVED_HEIGHT, MSG_SOLVED_POSITION))
    , m_msg_game_over(new MessagePanel(MSG_SOLVED_WIDTH, MSG_SOLVED_HEIGHT, MSG_SOLVED_POSITION))
    , m_msg_status(new MessagePanel(MSG_STATUS_WIDTH, MSG_STATUS_HEIGHT, MSG_STATUS_POSITION))
    , m_count(new Count(2, 0, COUNT_WIDTH, COUNT_HEIGHT, COUNT_POSITION))
{
    m_texture->add("pb_clear.png", "texture1");
    m_texture->add("pb_next.png", "texture2");
    m_texture->add("pb_help.png", "texture3");
...
void Toy::advance(int nanoseconds)
{
    ...
    while (m_seconds > seconds) {
        ...
        if (m_puzzle_book->current_challenge_solved()) {
            if (m_puzzle_book->current_challenge() == (m_puzzle_book->challenges() - 1) ) {
                m_msg_game_over->set_visible(true);
            } else {
                m_msg_solved->set_visible(true);
            }
        } else {
            m_msg_solved->set_visible(false);
            m_msg_game_over->set_visible(false);
        }
        m_msg_solved->set_visible(m_puzzle_book->current_challenge_solved());
        m_count->set_value(m_puzzle_book->current_challenge() + 1);
