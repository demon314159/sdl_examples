...
ConvexPlaneShape::ConvexPlaneShape(float radius, float angle_i, float angle_f, float height, int steps)
    : m_radius(radius)
    , m_angle_i(angle_i)
    , m_angle_f(angle_f)
    , m_height(height)
...
ConvexPlaneShape::~ConvexPlaneShape()
{
    if (m_facet != NULL)
        delete [] m_facet;
}

int ConvexPlaneShape::facets() const
{
    return m_facet_count;
}

Facet ConvexPlaneShape::facet(int facet_ix) const
{
    return m_facet[facet_ix];
}

void ConvexPlaneShape::define_shape()
{
...
void ConvexPlaneShape::slice(int step, float r, float theta_i, float theta_f)
{
    ...
    add_face({x1, y1, z1}, {x2, y1, z2}, {x2, y0, z2}, {x1, y0, z1}, true);
}

void ConvexPlaneShape::add_face(Float3 v1, Float3 v2, Float3 v3, Float3 v4, bool flip)
{
    add_face(v1, v2, v3, flip);
    add_face(v1, v3, v4, flip);
}

void ConvexPlaneShape::add_face(Float3 v1, Float3 v2, Float3 v3, bool flip)
{
    if (m_size_known) {
