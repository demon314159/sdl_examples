...
OpenBoxShape::OpenBoxShape(float dimx, float dimy, float dimz)
    : m_dimx(dimx)
    , m_dimy(dimy)
    , m_dimz(dimz)
    , m_size_known(false)
...
OpenBoxShape::~OpenBoxShape()
{
    if (m_facet != NULL)
        delete [] m_facet;
}

int OpenBoxShape::facets() const
{
    return m_facet_count;
}

Facet OpenBoxShape::facet(int facet_ix) const
{
    return m_facet[facet_ix];
}

void OpenBoxShape::define_shape()
{
    // faces
...
void OpenBoxShape::add_face(int v1, int v2, int v3, int v4, bool flip)
{
    add_face(v1, v2, v3, flip);
    add_face(v1, v3, v4, flip);
}

void OpenBoxShape::add_face(int v1, int v2, int v3, bool flip)
{
    if (m_size_known) {
...
Float3 OpenBoxShape::decode_vertex(int v)
{
    Float3 r;
