...
CadModel ThreePostKicker::model(float animation_id) const
{
    ...
    CadModel b4(PipeShape(rbumper, m_kicker4.length(), m_steps), m_color, animation_id);
    b4.translate(-m_kicker4.length() / 2.0, 0.0, rbumper);
    b4.rotate_ay(m_kicker4.angle());
    ...
    mm.add(b4, m_kicker4.position().v1, m_height, m_kicker4.position().v2);
