...
ConvexReflector::ConvexReflector(Float2 p1, float radius, float angle_i, float angle_f, float reflectivity)
    : m_perimeter({0.0, 0.0, 0.0, 0.0})
    , m_position(p1)
    , m_radius(radius)
    , m_angle_i(angle_i)
...
void ConvexReflector::update_perimeter()
{
...
bool ConvexReflector::null_velocity_origin() const
{
    return m_position.v1 == m_velocity_origin.v1 && m_position.v2 == m_velocity_origin.v2;
}
...
bool ConvexReflector::collide(Ball* ball) const
{
    bool flag = false;
